...
export const fetchToUpdateCar = createAsyncThunk(
    'cars/fetchToUpdateCar',
    async ({id, name, brandName, wheelPosition}: {
        id: string, 
        name: string, 
        brandName: string, 
        wheelPosition: WheelType
...
const carsSlice = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(fetchToCreateCar.fulfilled, (state, action) => {
            state.items.push(action.payload);
        })
    },
});
...
export const { setCars, setCurrentCar, addCar } = actions;
